...
-- ===== СВЕТ У ИГРОКА =====
local function setupPlayerLight()
	local char = player.Character or player.CharacterAdded:Wait()
	local hrp = char:WaitForChild("HumanoidRootPart")

	if hrp:FindFirstChild("PlayerLight") then return end

	local light = Instance.new("PointLight")
	light.Name = "PlayerLight"
	light.Range = 56
	light.Brightness = 1.5
	light.Color = Color3.fromRGB(218, 135, 51)
	light.Parent = hrp
end
...
-- ===== PA (для нажатия E) =====
local pa = workspace:WaitForChild("cursorp")
    :WaitForChild("cursor")
    :WaitForChild("p")
    :WaitForChild("pa")

local lastTransparency = pa.ImageTransparency

pa:GetPropertyChangedSignal("ImageTransparency"):Connect(function()
    if pa.ImageTransparency ~= lastTransparency then
        lastTransparency = pa.ImageTransparency
